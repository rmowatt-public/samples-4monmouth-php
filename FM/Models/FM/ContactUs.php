...
class FM_Models_FM_ContactUs extends FM_Models_BaseModel {

	protected $_tableName = 'contactus';
	...
	public function getRecordById($id) {
		$sql = "SELECT * from {$this->_dbName}.{$this->_tableName} c WHERE c.id = {$id}";
		return $this->getSingleRow($sql);
	}

	public function getRecordByKeys($keys){
		$where = $this->_makeWhere($keys, 'c');
		$sql = "SELECT * from {$this->_dbName}.{$this->_tableName} c WHERE {$where}";
		return $this->getSingleRow($sql);
	}
