...
class FM_Models_FM_MediaKit extends FM_Models_BaseModel {

	protected $_tableName = 'mediakit';
	...
	public function getRecordByKeys($keys){
		...
		//print $where; exit;
		$sql = "SELECT * from {$this->_dbName}.{$this->_tableName} m WHERE {$where} ORDER BY uploaded DESC";
		return $this->getSingleRow($sql);
	}
	
	public function getNewest() {
		$sql = "SELECT * from {$this->_dbName}.{$this->_tableName} m  ORDER BY uploaded DESC";
		return $this->getSingleRow($sql);
	}
