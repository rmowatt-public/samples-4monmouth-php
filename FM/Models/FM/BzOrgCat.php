...
class FM_Models_FM_BzorgCat extends FM_Models_BaseModel {

	protected  $_tableName = 'bzorg_cat';
	...
	public function getOrgNames($orgId) {
		$sql = "SELECT s.name from {$this->_dbName}.{$this->_tableName} u  JOIN {$this->_dbName}.search_primaryCategories s ON  u.catId = s.id WHERE u.orgId = {$orgId}";
		return $this->getMultipleRows($sql);
	}
	
	public function searchByCat($searchTerm) {
		$sql = "SELECT bz.orgId FROM {$this->_dbName}.{$this->_tableName} pc JOIN {$this->_dbName}.bzorg_cat bz ON pc.id = bz.catId WHERE pc.name LIKE '%{$searchTerm}%';";
		return $this->getMultipleRows($sql);
	}
}