...
class FM_Models_FM_PhotoAlbum extends FM_Models_BaseModel {


	protected $_tableName = 'photoAlbum';
	...
	public function getAlbumsByKeys($keys){
		$where = $this->_makeWhere($keys, 'b');
		$sql = "SELECT * from {$this->_dbName}.{$this->_tableName} b WHERE {$where} ORDER BY b.created DESC ";
		return $this->getMultipleRows($sql);
	}
	
	public function insertAlbum($args) {
		$keys = array();
		$values = array();
		$insert = array();
